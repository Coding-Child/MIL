...
def main(args):
    ...
    model = ResNetMIL(num_heads=num_heads, dropout=dropout)
    transform_train = T.Compose([T.RandomApply([T.RandomHorizontalFlip(p=0.5)], p=0.3),
                                 T.RandomApply([T.Lambda(lambda img: RandomRotation(img))], p=0.3),
                                 T.RandomApply([RandomErasing(fill_color=(225, 225, 225))], p=0.3),
                                 T.RandomApply([T.ElasticTransform(alpha=20.0, sigma=4.0, fill=225)], p=0.3),
                                 T.RandomApply([T.GaussianBlur(kernel_size=(5, 5), sigma=(0.1, 0.1))], p=0.3),
    ...
    dev_dataset = PathologyDataset(df=val_data, num_samples=test_num_sample, test=True, transform=transform_test)
