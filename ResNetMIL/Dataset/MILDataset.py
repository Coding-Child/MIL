...
def extract_patches(json_path, img_path, num_samples):
    ...
    # 사용 가능한 패치의 이름 목록을 가져옵니다.
    available_patch_names = list(patches_info.keys())

    # 사용 가능한 패치 수가 요청된 샘플 수보다 적은 경우, 모든 패치를 사용합니다.
    if num_samples is None or len(available_patch_names) <= num_samples:
        sampled_patch_names = available_patch_names
    else:
        # 그렇지 않으면 요청된 샘플 수만큼 무작위로 샘플링합니다.
        sampled_patch_names = random.sample(available_patch_names, num_samples)
...
class PathologyDataset(Dataset):
    def __init__(self, csv_file, num_samples, val=False, test=False, transform=None):
        ...
        if val:
            self.data_info = val_df
        elif test:
            self.data_info = test_df
        else:
            self.data_info = train_df
