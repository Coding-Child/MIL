    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(total=len(train_loader), desc=f'Epoch {epoch + 1}/{num_epochs}') as pbar:
            for i, (img, label, pseudo_label) in enumerate(train_loader):
                ...
                train_logger.info(
                    f"Step {i + 1} of epoch {epoch + 1}\nInstance Loss: {instance_loss.item():.10f}\nBag Loss: {bag_loss.item():.10f}")

                del img, label, pseudo_label, bag_logits, instance_logits, instance_loss, bag_loss
                gc.collect()
        ...
        if scheduler is not None:
            scheduler.step(valid_bag_loss)

        valid_logger.info(
            f"Epoch {epoch + 1}\nValidation AUC: {valid_auc:.4f}\nValidation F1 Score: {valid_f1:.4f}\nValidation Instance Loss: {valid_instance_loss.item():.10f}\nValidation Bag Loss: {valid_bag_loss.item():.10f}")
